...
class TestAppointmentHistory(unittest.TestCase):
    def setUp(self):
    ...
    def test_appointment_history_get(self):
        with self.app as client:
            with client.session_transaction() as session:
                session['_user_id'] = str(self.user.id)  

            result = client.get('/appointment_History', follow_redirects=True)

            self.assertEqual(result.status_code, 200)
            self.assertIn(b'appointment History', result.data)
